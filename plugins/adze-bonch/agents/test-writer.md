...
The orchestrator will include a `REPO_PATH` in your task prompt (e.g., `/home/user/workspaces/myproject`). Before doing any work, create an isolated worktree:

```bash
cd $REPO_PATH
BRANCH="worktree/$(date +%s)-$$"
WORKTREE_DIR="/tmp/adze-bonch-worktrees/$BRANCH"
